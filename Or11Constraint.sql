...
형식1] 인라인방식: 컬럼 생성시 우측에 제약조건을 기술한다.
    create table 테이블명(
        컬럼명 자료형(크기)[constrain 제약명] primary key
    )
...
제약조건 및 테이블목록 확인하기
    ...
    user_cons_columns: 테이블에 지정된 제약조건명과 컬럼명의 간략한
        정보를 저장한다.
    user_constraints: 테이블에 지정된 제약조건보다 상세한 정보를 
        저장한다.
...
desc tb_primary3;
/* 테이블을 생성한 후 alter문으로 제약조건을 부여한다. 제약명은 필요에따라
생략이 가능하다. */
...
--확인시 휴지통에 들어간것을 볼 수 있다.
select * from user_cons_columns;

--PK눈 테이블당 하나만 생성할 수 있으므로, 에러가 발생한다.
...
insert into tb_unique(idx, name, telephone, nickname)
    values(3, '슬기', '', '');
--unique는 중복을 허용하지 않지만 null을 여러개 입력할 수 있다.
...
create table tb_foreign1(
    ...
    f_name varchar2(50),
    /* 자식테이블인 tb_foreugn1에서 부모테이블인 tb_primary2의 user_id
    컬럼을 참조하는 외래키를 생성한다 */
...
create table tb_foreign2(
    ...
    idx컬럼을 참조하는 외래키를 설정한다. */
    foreign key (f_id) references tb_primary1(idx)
);
--데이터 저장
select * from user_constraints;
/*
데이터사전에서 제약조건 확인시의 플레그
p: Primary key 
R: Referece integrity 즉 Foreign key를 뜻한다.
C: Chek 혹은 Not null
U: Unique
*/
...
--부모키가 없으므로 레코드를 삽입할 수 없다. 오류 발생됨.
...
참조치 부분만 null값으로 변경된다. 따라서 더이상 참조할 수 없는 레코드로
변경된다.*/
...
create table tb_not_null(
    m_idx number(10) primary key, /* PK(primary key)이므로 NN(not null) */
    m_id varchar2(20) not null, /* NN */
    m_pw varchar2(30) null, /* null허용.(일반적으로 이렇게 쓰지않는다) */
...
--m_id가 NN으로 지정되었으므로 null값을 입력할 수 없다. 에러발생됨.
insert into tb_not_null values (4, '', '', '');
--입력성공. space도 문자이므로 입력된다.
insert into tb_not_null values (5, ' ', '5555', '오길동');
/*오류발생. PK로 지정된 컬럼은 null값을 입력할 수 없다. insert 쿼리에서
컬럼을 명시하지 않으면 null값이 입력된다. */
...
insert into tb_default values ('aaaa','1234');--1234 입력
insert into tb_default (id) values ('bbbb');--컬럼을 제외했으므로 default값 입력
...
/*
default값을 입력하면 insert문에서 컬럼 자체를 제외시키거나
default 키워드를 사용해야한다.
*/
...
--check 제약조건 위배로 오류발생됨
insert into tb_check1 values ('T');
--입력된 데이터가 컬럼의 크기보다 크므로 오류 발생됨
insert into tb_check1 values ('강아지');
--확인
